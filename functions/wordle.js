...
/**
 * Daily Wordle helpers for the Employee Portal Fun tab.
 * Answer of the day is deterministic from the Europe/London date key.
 */
...
const ANSWERS = [
  ...
  'below','bench','billy','birth','black','blame','blind','block','blood','blow',
  ...
  'every','exact','exist','extra','faith','false','fault','fibre','field','fifth',
  ...
  'young','youth','coach','route','shift','depot','tyres','fleet','drive','cabin',
  'brake','pedal','gauge','diesel','axle','radio','radar','alert','safe','check',
];

// Extra valid guesses beyond the answer list (common 5-letter words).
const EXTRA_GUESSES = [
  'aback','abate','abbey','abbot','abhor','abide','abled','abode','abort','abyss',
  'ached','acorn','acres','acrid','actor','acute','adage','adapt','adept','admin',
  'adore','adorn','affix','afoot','afoul','after','again','agape','agate','agent',
  'agile','aging','aglow','agony','agora','agree','ahead','aider','aisle','alarm',
  ...
  'curvy','cycle','cynic','daddy','daily','dairy','daisy','dally','dance','dandy',
  ...
  'gully','gumbo','gummy','guppy','gusto','gusty','habit','hairy','halve','handy',
  ...
  'logic','login','loopy','loose','lorry','loser','lossy','lotus','louse','lousy',
  ...
  'noise','noisy','nomad','noose','north','notch','noted','novel','nudge','nurse',
  'nutty','nylon','nymph','oaken','obese','occur','ocean','octal','octet','odder',
  'oddly','offal','offer','often','olden','older','olive','ombre','omega','onion',
  ...
  'panic','pansy','papal','paper','parer','parka','parry','parse','party','pasta',
  'paste','pasty','patch','patio','patsy','patty','pause','payee','payer','peace',
  'peach','pearl','pecan','pedal','penal','pence','penne','penny','perch','peril',
  ...
  'wince','winch','windy','wiser','wispy','witchch','witty','woken','woman','women',
...
const CLEAN_ANSWERS = [...new Set(ANSWERS.map(normalizeWord).filter((w) => w.length === 5))];
const VALID_SET = new Set([
  ...CLEAN_ANSWERS,
  ...EXTRA_GUESSES.map(normalizeWord).filter((w) => w.length === 5),
]);

function getAnswerForDay(dayKey = getLondonDayKey()) {
...
function isValidGuess(guess) {
  ...
  // Allow regular plurals as guesses (trees, books, doors) even if not in the dictionary.
  // Do not treat ss/us endings as automatic plurals (glass, focus).
  if (/^[a-z]{4}s$/.test(word) && !/(ss|us)$/.test(word)) return true;
  // Allow short -es plurals (boxes, foxes, taxes).
  if (/^[a-z]{3}es$/.test(word)) return true;
  return false;
}

/**
 * Classic Wordle evaluation with correct duplicate handling.
